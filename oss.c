...
while(1){
	...
	//Generate a new process every second and only generate 1 for now.
...
//If the flag is 1 that means we have an index available and we will generate a new process.
	if(flag == 1){
		if(fileLines < 10000){
			fprintf(filePtr, "\n\nTotal processes currently = %d launching new process\n\n\n", totalProcesses);
			fileLines += 6;
		}
...
if(totalProcesses > 0){
	...
	if((procArray[front].timeBlocked + 14000000) <= clock[1]){
		...
		buf.mtype = returnedProc.process;
	...
	if(buf.request == 99){
		fprintf("Process %d has terminated at time %d sec %d ns\n", buf.procIndex, clock[0], clock[1]);
		...
		for(x = 0; x < 32; x++){
			...
			if(pageTable[procIndex][x] != -1){
				frameTable[pageTable[procIndex][x]].occupied = 0;	
				frameTable[pageTable[procIndex][x]].dirty = -1;
				frameTable[pageTable[procIndex][x]].process = -1;
				frameTable[pageTable[procIndex][x]].page = -1;

				pageTable[procIndex][x] = -1;
			}				
	...
	if(fileLines < 10000 && buf.procIndex != 0){
        fprintf(filePtr,"Process %d has requested operation %d to memory location %d\n", buf.procIndex, buf.operation, buf.request);
        fileLines++;
        }
	...
	if(pageTable[buf.procIndex][pageNum] == -1){
		if(fileLines < 10000){
		fprintf(filePtr, "Page fault on process %d page %d", buf.procIndex, pageNum);
		}
		...
		if(fileLines < 10000 && buf.procIndex != 0){
        	fprintf(filePtr,"Granting process %d operation %d on memory location %d\n", buf.procIndex, buf.operation, buf.request);
       		fileLines++;
        	}
